...
from calc_core_py.schemas.enums import PlanetId, SignId
from calc_core_py.schemas.models import BodyPosition, ElementBalance, WesternSiderealCalcMode, WesternSiderealResult
...
def _lahiri_ayanamsha(dt: datetime) -> float:
    jd = _julian_day(dt)
    t = (jd - 2451545.0) / 36525.0
    lahiri = 23.85 + (50.2564 / 3600.0) * (jd - 2415020.0) / 36525.0 * 100
    return min(max(lahiri, 22), 25)
...
def calculate_sidereal(utc_dt: datetime, lat: float, lon: float) -> WesternSiderealResult:
    bodies: dict[PlanetId, BodyPosition] = {}
    element_counts = {element_id: 0 for element_id in ELEMENT_IDS}
    ayanamsha = _lahiri_ayanamsha(utc_dt)

    for index, planet_id in enumerate(PLANET_IDS):
        tropical_longitude = _longitude_for_planet(index, utc_dt, lat, lon)
        sidereal_longitude = (tropical_longitude - ayanamsha) % 360
        sign_id = _sign_for_longitude(sidereal_longitude)
        element_id = SIGN_TO_ELEMENT[sign_id]
        element_counts[element_id] += 1
        bodies[planet_id] = BodyPosition(
            longitude_deg=sidereal_longitude,
            sign_id=sign_id,
    ...
    return WesternSiderealResult(
        reference_frame_id="frame_02",
